...
function generateHardSemiprimes(): { n: bigint, p1: bigint, p2: bigint }[] {
  const primes = [
  ...
  const semiprimes: { n: bigint, p1: bigint, p2: bigint }[] = [];
...
async function benchmarkHardSemiprimes() {
  ...
  // Test each group
  for (const [name, group] of [['Small', small], ['Medium', medium], ['Large', large]] as const) {
    console.log(`\n--- ${name} Semiprimes ---`);
