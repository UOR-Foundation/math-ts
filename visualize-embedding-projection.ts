...
function visualizeProjection() {
  ...
  for (const { n, factors } of testNumbers) {
    ...
    const resonance = pattern.reduce((prod, active, i) => 
      active ? prod * constants[i] : prod, 1);
    ...
    for (let i = 2n; i < n; i++) {
      ...
      const iResonance = iPattern.reduce((prod, active, idx) => 
        active ? prod * constants[idx] : prod, 1);
