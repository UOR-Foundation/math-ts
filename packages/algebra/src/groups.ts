...
export class GroupDetector {
  ...
  private discoverEmergentGroups(numbers: bigint[]): GroupStructure[] {
    ...
    // 5. Discover artifact-born groups
  ...
  private discoverResonanceGroups(numbers: bigint[]): GroupStructure[] {
    ...
    for (const n of numbers) {
      ...
      resonanceMap.get(bucket)!.push(n);
    }
    
    // Form groups from resonance clusters
    for (const [resonance, elements] of resonanceMap) {
      if (elements.length >= 3) { // Need at least 3 for interesting structure
   ...
   */
  private crystallizeResonanceGroup(elements: bigint[], targetResonance: number): GroupStructure | null {
   ...
   */
  private computeMetabolicRate(elements: Set<bigint>, operation: (a: bigint, b: bigint) => bigint): number {
  ...
  private discoverFieldHarmonicGroups(numbers: bigint[]): GroupStructure[] {
    ...
    for (const n of numbers) {
      ...
      patternGroups.get(activeCount)!.push(n);
    }
  ...
  private discoverArtifactGroups(numbers: bigint[]): GroupStructure[] {
      ...
      if (factors.factors.length >= 2) {
        for (let i = 0; i < factors.factors.length - 1; i++) {
          ...
          if (typeof result === 'object' && 'artifacts' in result && result.artifacts) {
            const sig = `v${result.artifacts.vanishingFields.length}e${result.artifacts.emergentFields.length}`;
    ...
    for (const [sig, elements] of artifactMap) {
      if (elements.length >= 3) {
  ...
  private generateCyclicSubgroup(generator: bigint, parent: GroupStructure): GroupStructure {
    ...
    return {
      ...
      metabolicRate: parent.metabolicRate ? parent.metabolicRate * 0.8 : undefined,
    };
  }
  ...
  private reflectPattern(pattern: FieldPattern, axis: number): FieldPattern {
    ...
    switch (axis) {
      case 0: // Horizontal reflection
        return [result[0], result[7], result[6], result[5], result[4], result[3], result[2], result[1]];
      case 1: // Vertical reflection
        return [result[4], result[3], result[2], result[1], result[0], result[7], result[6], result[5]];
      case 2: // Diagonal reflection
        return [result[0], result[3], result[6], result[1], result[4], result[7], result[2], result[5]];
      case 3: // Anti-diagonal reflection
        return [result[0], result[5], result[2], result[7], result[4], result[1], result[6], result[3]];
      default:
        return result;
    }
  }
