...
describe('FixedPointEngine', () => {
  ...
  describe('Fixed Point Discovery', () => {
    ...
    it('should find digital root fixed points (1-9)', () => {
      ...
      const digitalRootFixed = analysis.fixedPoints.filter(fp => 
        fp.type === 'custom' && fp.value >= 1n && fp.value <= 9n
      );
    ...
    it('should find resonance fixed points', () => {
      ...
      const resonanceFixed = analysis.fixedPoints.filter(fp => 
        fp.type === 'custom' && fp.stability > 0.5 && fp.stability < 1.0
      );
  ...
  describe('Basin of Attraction', () => {
    ...
    it('should have larger basins for stronger attractors', () => {
      ...
      const weakAttractors = analysis.attractors.filter(fp => fp.stability <= 0.8 && fp.stability > 0.5);
      
      if (strongAttractors.length > 0 && weakAttractors.length > 0) {
        const avgStrongBasinSize = strongAttractors.reduce((sum, fp) => sum + fp.basin.length, 0) / strongAttractors.length;
        const avgWeakBasinSize = weakAttractors.reduce((sum, fp) => sum + fp.basin.length, 0) / weakAttractors.length;
