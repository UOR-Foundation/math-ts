...
async function testAlternativeProjections() {
  ...
  for (const { n, factors } of testSemiprimes) {
    ...
    console.log(`  Resonance (multiplicative): ${calculateResonance(substrate, n).toFixed(6)}`);
    
    // Alternative Projection 1: Additive resonance
    const additiveResonance = pattern.reduce((sum, active, i) => 
      sum + (active ? constants[i] : 0), 0);
    console.log(`  Additive resonance: ${additiveResonance.toFixed(6)}`);
    
    // Alternative Projection 2: Weighted by prime index
    const primeWeighted = pattern.reduce((sum, active, i) => 
      sum + (active ? constants[i] * (i + 1) : 0), 0);
    ...
    const euclideanNorm = Math.sqrt(pattern.reduce((sum, active, i) => 
      sum + (active ? constants[i] ** 2 : 0), 0));
    ...
    for (const factor of factors) {
      ...
      const fResonance = calculateResonance(substrate, factor);
      const fAdditive = fPattern.reduce((sum, active, i) => 
        sum + (active ? constants[i] : 0), 0);
  ...
  const metaResonance = calculateResonance(substrate, resonanceAsInt);
  ...
  const patternResonance = calculateResonance(substrate, patternAsNumber);
