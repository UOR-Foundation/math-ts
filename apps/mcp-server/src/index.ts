...
server.registerTool("analyze_number",
  {
  ...
  async ({ number }: { number: string | number }) => {
    try {
...
## Field Pattern
Active Fields: ${analysis.fields.map((active: boolean, i: number) => active ? ['I', 'N', 'T', 'φ', 'P', '∞', '½', 'ζ'][i] : null).filter(Boolean).join(', ')}
...
server.registerTool("find_primes",
  {
    title: "Find Primes",
  ...
  async ({ start, end, limit }: { start: string | number; end: string | number; limit: number }) => {
    try {
...
${primes.map(p => {
  ...
  return `${p}: Resonance ${analysis.resonance.toFixed(4)}, Fields: ${analysis.fields.map((active, i) => active ? ['I', 'N', 'T', 'φ', 'P', '∞', '½', 'ζ'][i] : null).filter(Boolean).join(', ')}`;
}).join('\n')}
`;
...
${result.fieldReconstruction.map((pattern: boolean[], i: number) => 
  `Step ${i}: ${pattern.map((b: boolean) => b ? '1' : '0').join('')}`
).join('\n')}

## Artifact Sources
${result.artifactSources.length > 0 ? 
  result.artifactSources.map((a: any) => `- Field ${a.field}: ${a.type}`).join('\n') : 
...
server.registerTool("explore_patterns",
  {
    ...
    inputSchema: {
      ...
      pattern: z.enum(["resonance", "fields", "stability", "lagrange"]).default("resonance").describe("Pattern type to explore")
    }
  },
  async ({ start, count, pattern }: { start: string | number; count: number; pattern: string }) => {
    try {
      const startN = BigInt(start);
      ...
      for (let i = 0n; i < count; i++) {
        ...
        switch (pattern) {
          case "resonance":
            ...
            break;
          case "fields":
            value = analysis.fields.map((active, i) => active ? ['I', 'N', 'T', 'φ', 'P', '∞', '½', 'ζ'][i] : null).filter(Boolean).join(',');
            break;
          case "stability":
            value = analysis.stabilityMetric.toFixed(4);
            break;
          case "lagrange":
            value = analysis.isLagrangePoint ? "🌟 LAGRANGE" : `dist: ${analysis.distanceToLagrange}`;
            break;
        }
...
server.registerTool("resonance_landscape",
  {
    ...
    inputSchema: {
      ...
      dimensions: z.array(z.enum(["arithmetic", "field", "page"])).default(["arithmetic"]).describe("Dimensions to explore")
    }
  },
  async ({ center, radius, dimensions }: { center: string | number; radius: number; dimensions: string[] }) => {
    try {
...
## Landscape Features
- Maximum resonance: ${Math.max(...landscape.map((p: any) => p.resonance)).toFixed(4)}
- Minimum resonance: ${Math.min(...landscape.map((p: any) => p.resonance)).toFixed(4)}
- Average resonance: ${(landscape.reduce((sum: number, p: any) => sum + p.resonance, 0) / landscape.length).toFixed(4)}
`;
...
server.registerTool("lagrange_navigation",
  {
  ...
  async ({ start, target, maxHops }: { start: string | number; target: string | number; maxHops: number }) => {
    try {
      const startN = BigInt(start);
...
server.registerTool("field_archaeology",
  {
  ...
  async ({ number, depth }: { number: string | number; depth: string }) => {
    try {
      const n = BigInt(number);
      const analysis = universe.analyze(n);
      ...
      if (depth === "surface" || depth === "deep" || depth === "quantum") {
        ...
        artifacts.push(`Field Pattern: ${analysis.fields.map((active, i) => active ? `${i}:${['I', 'N', 'T', 'φ', 'P', '∞', '½', 'ζ'][i]}` : null).filter(Boolean).join(', ')}`);
...
server.registerTool("universe_health",
  {
    title: "Universe Health",
    description: "Check Mathematical Universe system health and consistency",
    inputSchema: {
      checkType: z.enum(["quick", "full", "conservation"]).default("quick").describe("Type of health check")
    }
  },
  async ({ checkType }: { checkType: string }) => {
    try {
      ...
      healthReport.push(`- Field constants: ${universe.getFieldConstants().map(c => c.toFixed(4)).join(', ')}`);
